...
function getProfileInfo(profileUrl, result) {
  console.log("LinkedIn Profil Analizci başlatıldı...");

  // result nesnesi burada parametre olarak alınıyor, bu yüzden yeniden tanımlamaya gerek yok
  // const result = {
//     name: "Ad bulunamadı",
...
function logToConsole(message) {
  // Send logs to the background script for visibility
  chrome.runtime.sendMessage({ action: "log", message });
  console.log(message); // Also log to the content script console
}

chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "getProfileData") {
    logToConsole("Content script: getProfileData message received.");
    try {
      const profileId = request.profileId; // Kullanıcı ID'si alınıyor
      const profileUrl = `https://www.linkedin.com/in/${profileId}`; // Profil URL'si oluşturuluyor
      const recentActivityUrl = `${profileUrl}/recent-activity/all/`; // Postlar için URL oluşturuluyor
      ...
      logToConsole(`Profile URL: ${profileUrl}`);
      logToConsole(`Recent Activity URL: ${recentActivityUrl}`);

      // Profil bilgilerini çek
      getProfileInfo(profileUrl, result);

      // Postları çek - async işlem
      fetchRecentActivityPosts(recentActivityUrl, result, () => {
        // İşlem tamamlandıktan sonra response gönder
        sendResponse({ status: "success", data: result });
      });

    } catch (error) {
      logToConsole(`Content script: Error fetching profile data: ${error}`);
      sendResponse({
        status: "error",
        message: error?.message || "Bilinmeyen hata",
      });
    }
    return true; // Asenkron response için true döndür
  }
});
...
function processPost(postElement, index, result) {
  try {
    ...
    const timeSelectors = [
      ...
      ".t-12.t-black--light",
      "time[datetime]",
      ".update-components-update-actor__sub-description time",
    ];
...
function fetchRecentActivityPosts(recentActivityUrl, result, callback) {
  try {
    logToConsole(`Opening new tab for recent activity: ${recentActivityUrl}`);
    
    // Yeni tab aç ve oradaki DOM'dan veriyi oku
    chrome.tabs.create({ url: recentActivityUrl, active: false }, (tab) => {
      // Tab yüklendiğinde çalışacak listener
      chrome.tabs.onUpdated.addListener(function listener(tabId, changeInfo) {
        if (tabId === tab.id && changeInfo.status === 'complete') {
          // Listener'ı kaldır
          chrome.tabs.onUpdated.removeListener(listener);
          
          // Content script'i tab'a inject et
          chrome.scripting.executeScript({
            target: { tabId: tab.id },
            func: () => {
              // Bu fonksiyon yeni tab'da çalışacak
              const posts = [];
              
              // Post elementlerini bul
              const postElements = document.querySelectorAll(".feed-shared-update-v2[data-urn], li.occludable-update[data-urn]");
              ...
              postElements.forEach((postElement, index) => {
                if (posts.length < 20) {
                  ...
                  if (timeElement) {
                    post.timeAgo = timeElement.textContent.trim();
                  }
            ...
            if (results && results[0] && results[0].result) {
              result.posts = results[0].result;
              logToConsole(`Posts fetched successfully: ${result.posts.length} posts found`);
            }
    ...
    logToConsole(`Error in fetchRecentActivityPosts function: ${error.message}`);
    if (callback) {
      callback();
    }
  }
